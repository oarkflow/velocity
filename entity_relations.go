...
func isValidEntityType(entityType string) bool {
	...
	case EntityTypeJSON, EntityTypeSecret, EntityTypeObject, EntityTypeFolder, EntityTypeCustom:
		return true
	default:
		// Allow any non-empty string as entity type for extensibility
		// (domain types like person, organization, account, etc.)
		return entityType != ""
	}
}
...
func isValidRelationType(relationType string) bool {
	...
	case RelationTypeContains, RelationTypeReferences, RelationTypeDependsOn,
		...
		return true
	default:
		// Allow any non-empty string as relation type for extensibility
		// (domain relations like works_for, owns, suspect_in, etc.)
		return relationType != ""
	}
}
