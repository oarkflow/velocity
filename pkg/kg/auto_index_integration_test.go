...
func newAutoKGTestDB(t *testing.T, resources ...kg.ResourceType) *DB {
	t.Helper()
...
func requireKGHits(t *testing.T, db *DB, query string, want int) *kg.KGSearchResponse {
	t.Helper()
...
func TestKnowledgeGraphAutoIndex_DisabledByDefault(t *testing.T) {
	...
	resp, err := db.KnowledgeGraph().Search(context.Background(), &kg.KGSearchRequest{Query: "alpha", Limit: 10})
	if err != nil {
...
func TestKnowledgeGraphAutoIndex_KVExistingNewDeleteAndRecursion(t *testing.T) {
	db := newAutoKGTestDB(t, kg.ResourceKV)
	...
	resp, err := db.KnowledgeGraph().Search(context.Background(), &kg.KGSearchRequest{Query: "audit trail", Limit: 10})
	if err != nil {
	...
	if err := db.Put([]byte("__kg:doc:manual"), []byte("recursive should not index")); err != nil {
		t.Fatalf("put kg key: %v", err)
	}
	resp, err = db.KnowledgeGraph().Search(context.Background(), &kg.KGSearchRequest{Query: "recursive", Limit: 10})
	if err != nil {
...
func TestKnowledgeGraphAutoIndex_ObjectSecretEnvelopeEntity(t *testing.T) {
	...
	db := newAutoKGTestDB(t, kg.ResourceObject, kg.ResourceSecret, kg.ResourceEnvelope, kg.ResourceEntity)
	...
	resp, err := db.KnowledgeGraph().Search(ctx, &kg.KGSearchRequest{Query: "entity searchable", Limit: 10})
	if err != nil {
...
func TestKnowledgeGraphAutoIndex_SecretMetadataOnlyMode(t *testing.T) {
	...
	db.EnableKnowledgeGraphAutoIndex(KnowledgeGraphAutoIndexConfig{
		Enabled:       true,
		Resources:     []kg.ResourceType{kg.ResourceSecret},
		SecretValues:  false,
		Existing:      false,
		Async:         false,
		MaxValueBytes: 1024,
	...
	resp, err := db.KnowledgeGraph().Search(ctx, &kg.KGSearchRequest{Query: "do-not-index-secret-value", Limit: 10})
	if err != nil {
...
func TestKnowledgeGraphResourceGraph_SearchConnectsResources(t *testing.T) {
	db := newAutoKGTestDB(t, kg.ResourceKV, kg.ResourceObject)
	...
	graph, err := db.KnowledgeGraph().SearchResourceGraph(context.Background(), &kg.KGResourceGraphRequest{
...
func TestKnowledgeGraphAutoIndex_BackgroundConfigAndResourceFilter(t *testing.T) {
	...
	reopened, err := NewWithConfig(Config{
		...
		KnowledgeGraphAutoIndexResources: []kg.ResourceType{kg.ResourceKV},
	})
	if err != nil {
	...
	resp, err := reopened.KnowledgeGraph().Search(context.Background(), &kg.KGSearchRequest{Query: "object should not sync", Limit: 10})
	if err != nil {
