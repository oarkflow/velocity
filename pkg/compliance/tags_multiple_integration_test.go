...
func TestComplianceTagManager_MultipleTagsUpdate(t *testing.T) {
	...
	for _, tag := range updatedTags {
		...
		if tag.Frameworks[0] == compliance.FrameworkSOC2 && tag.RetentionDays == 730 {
			soc2Unchanged = true
		}
	}
	if !gdprUpdated {
	...
	err = ctm.UpdateTagByPathAndFramework(ctx, "/customer-data", compliance.FrameworkSOC2, func(tag *ComplianceTag) error {
	...
	updatedTags = ctm.GetTags("/customer-data")
	for _, tag := range updatedTags {
		if tag.Frameworks[0] == compliance.FrameworkSOC2 && tag.EncryptionReq {
			soc2Updated = true
		}
	}
	if !soc2Updated {
	...
	if remainingTags[0].Frameworks[0] != compliance.FrameworkSOC2 {
...
func TestComplianceTagManager_UpdateErrors(t *testing.T) {
	...
	err = ctm.UpdateTagByPathAndFramework(ctx, "/non-existent", compliance.FrameworkGDPR, func(tag *ComplianceTag) error {
		return nil
	})
	if err == nil {
	...
	err = ctm.UpdateTagByPathAndFramework(ctx, "/test-data", compliance.FrameworkHIPAA, func(tag *ComplianceTag) error {
		return nil
	})
	if err == nil {
