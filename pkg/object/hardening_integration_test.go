...
func TestObjectHardeningChecksumCorruptionFailsClosed(t *testing.T) {
	dbPath := t.TempDir()
	db, err := NewWithConfig(Config{Path: dbPath, DisableEncryption: true})
	if err != nil {
		t.Fatalf("NewWithConfig: %v", err)
	}
	t.Cleanup(func() { _ = db.Close() })
	...
	objectPath := filepath.Join(dbPath, "objects", "objects", rec.ObjectID, rec.VersionID)
...
func TestS3CredentialStoreDoesNotPersistPlaintextSecretWhenEncrypted(t *testing.T) {
	...
	store := s3.NewS3CredentialStore(db, db)
	cred, err := store.GenerateCredentials("alice", "test")
	if err != nil {
		t.Fatalf("GenerateCredentials: %v", err)
	}
	raw, err := db.Get([]byte("s3:cred:" + cred.AccessKeyID))
	if err != nil {
...
func TestPresignedURLValidationRejectsTampering(t *testing.T) {
	...
	pg := s3.NewPresignedURLGenerator(store, "us-east-1", "http://localhost:8080")
