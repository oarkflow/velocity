...
func TestTOTPConfig_Defaults(t *testing.T) {
	...
	cfg := mfa.TOTPConfig()
	if cfg.Issuer != "Velocity Database" {
		t.Errorf("Expected issuer 'Velocity Database', got %s", cfg.Issuer)
	}
	...
	if cfg.Digits != 6 {
		t.Errorf("Expected 6 digits, got %d", cfg.Digits)
	}

	if cfg.Period != 30 {
		t.Errorf("Expected 30 second period, got %d", cfg.Period)
	}

	if cfg.Skew != 1 {
		t.Errorf("Expected skew of 1, got %d", cfg.Skew)
	}
}
