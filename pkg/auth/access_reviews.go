...
func NewAccessReviewManager(store KVStore) *AccessReviewManager {
	return &AccessReviewManager{store: store}
}
...
func (arm *AccessReviewManager) CreateReview(ctx context.Context, review *AccessReview) error {
	...
	return arm.store.Put([]byte("access:review:"+review.ReviewID), data)
}
...
func (arm *AccessReviewManager) CompleteReview(ctx context.Context, reviewID, status, notes string) error {
	data, err := arm.store.Get([]byte("access:review:" + reviewID))
	if err != nil {
