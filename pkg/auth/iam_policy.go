...
func NewIAMPolicyEngine(store KVStore) *IAMPolicyEngine {
	return &IAMPolicyEngine{store: store}
}
...
func (e *IAMPolicyEngine) CreatePolicy(policy *IAMPolicy) error {
	...
	return e.store.Put([]byte(iamPolicyPrefix+policy.Name), data)
}
...
func (e *IAMPolicyEngine) DeletePolicy(name string) error {
	e.mu.Lock()
	defer e.mu.Unlock()
	return e.store.Delete([]byte(iamPolicyPrefix + name))
}
...
func (e *IAMPolicyEngine) GetPolicy(name string) (*IAMPolicy, error) {
	...
	data, err := e.store.Get([]byte(iamPolicyPrefix + name))
	if err != nil {
...
func (e *IAMPolicyEngine) ListPolicies() ([]*IAMPolicy, error) {
	...
	keys, err := e.store.Keys(iamPolicyPrefix + "*")
	if err != nil {
...
func (e *IAMPolicyEngine) AttachUserPolicy(userID, policyName string) error {
	...
	if !e.store.Has([]byte(iamPolicyPrefix + policyName)) {
...
func (e *IAMPolicyEngine) EvaluateAccess(req *IAMEvalRequest) *IAMEvalResult {
	...
	for _, pName := range policyNames {
		data, err := e.store.Get([]byte(iamPolicyPrefix + pName))
		if err != nil {
			continue
		}
		var policy IAMPolicy
...
func (e *IAMPolicyEngine) collectPoliciesForPrincipal(principal string) []string {
	...
	keys, err := e.store.Keys(iamGroupPrefix + "*")
	if err == nil {
		for _, key := range keys {
			data, _ := e.store.Get([]byte(key))
			if data == nil {
				continue
			}
...
func (e *IAMPolicyEngine) getUserAttachment(userID string) *IAMUserAttachment {
	data, err := e.store.Get([]byte(iamUserPrefix + userID))
	if err != nil || data == nil {
...
func (e *IAMPolicyEngine) saveUserAttachment(att *IAMUserAttachment) error {
	...
	return e.store.Put([]byte(iamUserPrefix+att.UserID), data)
}
...
func (e *IAMPolicyEngine) getGroupAttachment(groupID string) *IAMGroupAttachment {
	data, err := e.store.Get([]byte(iamGroupPrefix + groupID))
	if err != nil || data == nil {
...
func (e *IAMPolicyEngine) saveGroupAttachment(att *IAMGroupAttachment) error {
	...
	return e.store.Put([]byte(iamGroupPrefix+att.GroupID), data)
}
