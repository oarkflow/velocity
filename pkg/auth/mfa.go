...
// TOTPConfig returns the active TOTP settings.
func (mfa *MFAManager) TOTPConfig() TOTPConfig {
	if mfa == nil || mfa.totpConfig == nil {
		return TOTPConfig{}
	}
	return *mfa.totpConfig
}
...
// GenerateBackupCodes generates one-time backup codes.
func (mfa *MFAManager) GenerateBackupCodes(count int) ([]string, error) {
	return mfa.generateBackupCodes(count)
}
