...
const BucketMetaPrefix = bucketMetaPrefix
...
func NewBucketManager(store KVStore, objects ...ObjectLister) *BucketManager {
	bm := &BucketManager{store: store}
	if len(objects) > 0 {
		bm.objects = objects[0]
	}
	return bm
}
...
func (bm *BucketManager) CreateBucket(name, owner, region string) error {
	...
	if bm.store.Has([]byte(bucketMetaPrefix + name)) {
	...
	if err := bm.store.PutWithTTL([]byte(bucketMetaPrefix+name), data, 0); err != nil {
		return err
	}
	...
	return bm.store.PutWithTTL([]byte(bucketConfigPrefix+name), configData, 0)
}
...
func (bm *BucketManager) DeleteBucket(name string) error {
	...
	if bm.objects != nil {
		count, err := bm.objects.ListObjectsForBucket(name+"/", 1)
		if err != nil {
			return err
		}
		if count > 0 {
	...
	bm.store.Delete([]byte(bucketMetaPrefix + name))
	bm.store.Delete([]byte(bucketConfigPrefix + name))
	bm.store.Delete([]byte(bucketPolicyPrefix + name))
	bm.store.Delete([]byte(bucketQuotaPrefix + name))
...
func (bm *BucketManager) HeadBucket(name string) (*BucketInfo, error) {
	data, err := bm.store.Get([]byte(bucketMetaPrefix + name))
	if err != nil {
...
func (bm *BucketManager) ListBuckets(owner string) ([]BucketInfo, error) {
	keys, err := bm.store.Keys(bucketMetaPrefix + "*")
	if err != nil {
		return nil, err
	}
	...
	for _, key := range keys {
		data, err := bm.store.Get([]byte(key))
		if err != nil {
			continue
		}
...
func (bm *BucketManager) GetBucketConfig(name string) (*BucketConfig, error) {
	data, err := bm.store.Get([]byte(bucketConfigPrefix + name))
	if err != nil {
...
func (bm *BucketManager) SetBucketConfig(name string, config *BucketConfig) error {
	...
	return bm.store.PutWithTTL([]byte(bucketConfigPrefix+name), data, 0)
}
...
func (bm *BucketManager) SetBucketPolicy(name string, policy json.RawMessage) error {
	...
	return bm.store.PutWithTTL([]byte(bucketPolicyPrefix+name), policy, 0)
}

// GetBucketPolicy retrieves bucket policy
func (bm *BucketManager) GetBucketPolicy(name string) (json.RawMessage, error) {
	data, err := bm.store.Get([]byte(bucketPolicyPrefix + name))
	if err != nil {
...
func (bm *BucketManager) DeleteBucketPolicy(name string) error {
	return bm.store.Delete([]byte(bucketPolicyPrefix + name))
}
...
func (bm *BucketManager) SetBucketQuota(name string, quota *BucketQuota) error {
	...
	return bm.store.PutWithTTL([]byte(bucketQuotaPrefix+name), data, 0)
}

// GetBucketQuota retrieves bucket quota
func (bm *BucketManager) GetBucketQuota(name string) (*BucketQuota, error) {
	data, err := bm.store.Get([]byte(bucketQuotaPrefix + name))
	if err != nil {
