...
func NewS3CredentialStore(store KVStore, codec ...SecretCodec) *S3CredentialStore {
	cs := &S3CredentialStore{store: store}
	if len(codec) > 0 {
		cs.codec = codec[0]
	}
	return cs
...
func (cs *S3CredentialStore) GenerateCredentials(userID, description string) (*S3Credential, error) {
	...
	if err := cs.store.PutWithTTL([]byte(credPrefix+accessKey), data, 0); err != nil {
		return nil, err
	}
...
func (cs *S3CredentialStore) GetCredential(accessKeyID string) (*S3Credential, error) {
	data, err := cs.store.Get([]byte(credPrefix + accessKeyID))
	if err != nil {
...
func (cs *S3CredentialStore) DeleteCredential(accessKeyID string) error {
	...
	return cs.store.PutWithTTL([]byte(credPrefix+accessKeyID), data, 0)
}
...
func (cs *S3CredentialStore) ListCredentials(userID string) ([]*S3Credential, error) {
	keys, err := cs.store.Keys(credPrefix + "*")
	if err != nil {
		return nil, err
	}
	...
	for _, key := range keys {
		data, err := cs.store.Get([]byte(key))
		if err != nil {
			continue
		}
...
func (cs *S3CredentialStore) sealCredentialSecret(cred *S3Credential) error {
	...
	if cs.codec != nil && cs.codec.HasCredentialEncryption() {
		encrypted, err := cs.codec.EncryptCredentialSecret(cred.AccessKeyID, cred.SecretAccessKey)
		if err != nil {
			return err
		}
...
func (cs *S3CredentialStore) openCredentialSecret(cred *S3Credential) error {
	...
	plain, err := cs.codec.DecryptCredentialSecret(cred.AccessKeyID, cred.EncryptedSecret)
	if err != nil {
		return err
	}
	cred.SecretAccessKey = plain
	return nil
}
