# ROLE Command Deep-Dive

This document is implementation-oriented guidance for the `secretr role` command group.
...
| Subcommand | Purpose |
|---|---|
| `secretr role` | Role-based access control |
| `secretr role assign` | Assign role to identity |
| `secretr role create` | Create a role |
| `secretr role list` | List roles |
...
|---|---|
| `secretr role` | `role:read` |
| `secretr role assign` | `role:assign` |
| `secretr role create` | `role:create` |
| `secretr role list` | `role:read` |

## 3. ACL / Resource Model

| Subcommand | Resource Type | ACL Required |
|---|---|---|
| `secretr role` | `role` | yes |
| `secretr role assign` | `role` | yes |
| `secretr role create` | `role` | yes |
| `secretr role list` | `role` | yes |
...
|---|---|---|---|---|---|---|
| `--identity` | - | `string` | yes | `resource_selector` | yes | Identity ID |
| `--role` | - | `string` | yes | `control` | no | Role ID |
...
| Flag | Aliases | Type | Required | Auth Class | ACL Required | Description |
|---|---|---|---|---|---|---|
| `--description` | `-d` | `string` | no | `control` | no | Description |
| `--name` | `-n` | `string` | yes | `resource_selector` | yes | Role name |
| `--scopes` | `-s` | `string[]` | no | `control` | no | Scopes |
