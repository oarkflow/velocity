...
func TestMemoryVFS_NoTempFileCreation(t *testing.T) {
	...
	// Skip the actual ViewFolderSecure call as it starts a blocking preview server.
	// The non-blocking VFS loading is already tested in TestViewFolderSecure_Integration.
	t.Log("Skipping ViewFolderSecure (blocks on preview server); VFS loading tested separately")
